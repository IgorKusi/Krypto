...
public class Util {
    ...
    public static String bytes_to_numeric(byte[] bytes) {
        ...
        for (byte b : bytes) {
            sb.append(byte_to_numeric(b)).append(" ");
        }
    ...
    public static BigInteger[] strToBIArray(String str) {
        str = str.substring(1, str.length() - 1);
        String[] arr = str.split(",");

        BigInteger[] ret = new BigInteger[ arr.length ];

        for (int i = 0; i < ret.length; i++) {
            ret[ i ] = new BigInteger(arr[ i ].trim());
        }
